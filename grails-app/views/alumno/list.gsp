			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="padron" title="${message(code: 'alumno.padron.label', default: 'Padron')}" />
					
						<g:sortableColumn property="sexo" title="${message(code: 'alumno.sexo.label', default: 'Sexo')}" />
					
						<g:sortableColumn property="saldo" title="${message(code: 'alumno.saldo.label', default: 'Saldo')}" />
				...
				<g:each in="${alumnoInstanceList}" status="i" var="alumnoInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: alumnoInstance, field: "padron")}</td>
					
						<td>${fieldValue(bean: alumnoInstance, field: "sexo")}</td>
					
						<td>${fieldValue(bean: alumnoInstance, field: "saldo")}</td>
